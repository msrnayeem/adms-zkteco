        ...
        <form method="post" action="{{ route('shifts.store') }}">
            ...
            <div class="form-group mt-2">
                ...
                <input type="text" name="name" class="form-control @error('name') is-invalid @enderror" id="name"
                    placeholder="Enter shift name" value="{{ old('name') }}">
                @error('name')
            ...
            <div class="form-group">
                ...
                @endforeach
                <button type="button" class="btn btn-sm btn-outline-primary" id="toggle-days">Mark All
                    Days</button>
                @foreach ($days as $day)
                    @error($day)
                        <div class="text-danger">{{ $message }}</div>
            ...
            <div class="form-group mt-2">
                ...
                <input type="time" name="entry_time" class="form-control @error('entry_time') is-invalid @enderror"
                    id="entry_time" value="{{ old('entry_time', '10:00') }}">
                @error('entry_time')
                    <div class="text-danger">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="form-group mt-2">
                ...
                <input type="number" name="late_entry" class="form-control @error('late_entry') is-invalid @enderror"
                    id="late_entry" placeholder="Enter late entry minutes" value="{{ old('late_entry', 15) }}">
                @error('late_entry')
                    <div class="text-danger">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="form-group mt-2">
                ...
                <input type="time" name="out_time" class="form-control @error('out_time') is-invalid @enderror"
                    id="out_time" value="{{ old('out_time', '18:00') }}">
                @error('out_time')
                    <div class="text-danger">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="form-group mt-2">
                ...
                <input type="number" name="early_out_time"
                    ...
                    placeholder="Enter early out minutes" value="{{ old('early_out_time', 15) }}">
